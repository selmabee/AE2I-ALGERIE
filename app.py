...
if not SUPABASE_URL or not SUPABASE_KEY:
    ...
    print("[ERROR] Missing configuration - Supabase client not initialized")
else:
    try:
        supabase = create_client(SUPABASE_URL, SUPABASE_KEY)
        print("[INFO] Supabase connection established successfully")
...
LINKEDIN_REDIRECT_URI = os.environ.get('LINKEDIN_REDIRECT_URI', 'http://localhost:5000/api/linkedin_callback')
...
if __name__ == '__main__':
    ...
    print(f"[INFO] Flask app starting on port {port}")
    if supabase is None:
        print("[WARNING] Starting without Supabase connection")
